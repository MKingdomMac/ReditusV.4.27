...
void URadioSelect::SetOptions(TArray<FSettingOption> InOptions)
{
	...
	if (IsValid(ButtonContainer))
	{
		...
		// Set initial keyboard/gamepad focus to the first radio button at runtime.
		// Skip in designer preview to avoid editor focus stealing.
		if (!IsDesignTime() && RadioButtons.Num() > 0)
		{
			APlayerController* PC = GetOwningPlayer();
			if (PC && RadioButtons[0]->IsVisible())
			{
				RadioButtons[0]->SetUserFocus(PC);
			}
		}
	}
